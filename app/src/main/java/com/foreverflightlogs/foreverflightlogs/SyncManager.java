...
public class SyncManager {
  ...
  /**
   * Create a json object by taking items stored in the db and set those values into an object to
   * pass to the API UserFlights { "userID": int, "flights" [ { flight info "segments" [ {
   * segmentInfo } ] }, { flight #2 info segments for flight#2 [ { segment info } ] } ] }
   */
  public static String createJson(Context context) {
    ...
    for (Flight dbflight : dbflights) {
      ...
      for (Segment dbsegment : segmentPresenter.segments) {
        ...
        jsonSegment
            .setSimulatedInstrument(convertBooleanToString(dbsegment.getSimulatedInstruments()));
        jsonSegment
            .setInstrumentFlightRules(convertBooleanToString(dbsegment.getinstrumentFlight()));
